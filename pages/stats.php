...
<div class="col-xs-12">
    ...
    <?php $graphCdnRequest = array(); ?>
    <?php foreach ($cdnRequest as $values): ?>
        <?php $dt = new DateTime($values->date); ?>
        <?php $graphCdnRequest[] = array($dt->format('U') * 1000, $values->value) ?>
     <?php endforeach; ?>
     <?php $graphCdnRequest = array_reverse($graphCdnRequest) ?>
    
    <?php $graphBackendRequest = array(); ?>
    <?php foreach ($backendRequest as $values): ?>
        <?php $dt = new DateTime($values->date); ?>
        <?php $graphBackendRequest[] = array($dt->format('U') * 1000, $values->value) ?>
     <?php endforeach; ?>
     <?php $graphBackendRequest = array_reverse($graphBackendRequest) ?>
    
    <?php $graphCdnBandwidth = array(); ?>
    <?php foreach ($cdnBandwidth as $values): ?>
        <?php $dt = new DateTime($values->date); ?>
        <?php $graphCdnBandwidth[] = array($dt->format('U') * 1000, formatBytes($values->value)) ?>
     <?php endforeach; ?>
     <?php $graphCdnBandwidth = array_reverse($graphCdnBandwidth) ?>
    
    <?php $graphBackendBandwidth = array(); ?>
    <?php foreach ($backendBandwidth as $values): ?>
        <?php $dt = new DateTime($values->date); ?>
        <?php $graphBackendBandwidth[] = array($dt->format('U') * 1000, formatBytes($values->value)) ?>
     <?php endforeach; ?>
